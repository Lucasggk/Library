	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:AddSubTab(Title, Icon)
			...
			function SubTab:AddSection(SectionTitle, SectionIcon)
				local Section = { Type = "Section", Name = Tab.Name, TabName = Tab.Name, NoSave = Tab.NoSave }
...
function Window:AddTab(TabConfig)
			...
			tab.NoSave = TabConfig.SaveManager == false
...
ElementsTable.Discord = (function()
	...
	local requestFunc = request or http_request or syn and syn.request or nil

	local function extractCode(url)
	...
	local function fetchDiscordData(code)
		...
		local ok2, data = pcall(game:GetService("HttpService").JSONDecode, game:GetService("HttpService"), res.Body)
		if not ok2 then return nil end
		return data
	end

	local function downloadIcon(guildId, iconHash)
		if not requestFunc then return nil end
		local url = "https://cdn.discordapp.com/icons/" .. guildId .. "/" .. iconHash .. ".png"
		local fileName = "discord_icon_" .. guildId .. ".png"
		local ok, res = pcall(requestFunc, { Url = url, Method = "GET" })
		if not ok or not res then return nil end
		if hasWritefile then pcall(writefile, fileName, res.Body) end
		if hasGetcustomasset and hasIsfile then
			local asset
	...
	function Element:New(Config)
		local code = extractCode(Config.Discord)
		local corner = Config.Corner or UDim.new(0, 4)
		local inviteUrl = code and ("https://discord.gg/" .. code) or ""
		local New = Creator.New
		...
		Creator.AddSignal(discordBtn.MouseButton1Click, function()
			if setclipboard then setclipboard(inviteUrl) end
		end)

		local function update()
			if not code then return end
			local data = fetchDiscordData(code)
			if not data then return end
			local guild = data.guild
			local profile = data.profile
			if not guild or not profile then return end
			local total = profile.member_count or data.approximate_member_count or 0
			local online = profile.online_count or data.approximate_presence_count or 0
			nameLabel.Text = profile.name or guild.name or "Unknown"
			onlineLabel.Text = tostring(online) .. " online"
			totalLabel.Text = tostring(total) .. " membros"
			if guild.icon then
				local asset = downloadIcon(guild.id, guild.icon)
				if asset then
					iconImage.Image = asset
				else
					iconImage.Image = "https://cdn.discordapp.com/icons/" .. guild.id .. "/" .. guild.icon .. ".png"
				end
			end
		end
...
for _, ElementComponent in pairs(ElementsTable) do
	Elements["Add" .. ElementComponent.__type] = function(self, Idx, Config)
		...
		if type(Idx) == "table" then
			Config = Idx
			local candidate = Config.Id or Config.Title
			if type(candidate) == "string" and candidate ~= "" then
				IdxStr = candidate
			else
				IdxStr = tostring(math.random(100000, 999999))
			end
		else
		...
		if eType == "Button" or eType == "Paragraph" or eType == "Discord" then
			...
			if result then
				...
				if SaveManager and not self.NoSave then
...
SaveManager = {
	...
	Parser = {
		...
		Slider = {
			...
			Load = function(idx, data)
				if Library.Options[idx] then Library.Options[idx]:SetValue(tonumber(data.value) or data.value, true) end
			end,
		},
		Dropdown = {
			Save = function(idx, object)
...
function SaveManager:BuildData()
	...
	for idx, option in next, Library.Options do
		if not self.Ignore[idx] and not option.NoSave and self.Parser[option.Type] then
...
function SaveManager:Save()
	if not HasFileSystem() then return false, "No file system available" end
...
function SaveManager:Load()
	...
	if not applyOk then return false, applyErr end
	return true
end
...
function SaveManager:HookOption(option)
	...
	option.Callback = function(...)
		...
		if SaveManager._isLoading then return end
...
function SaveManager:ClearSave()
	local ok, err = pcall(function()
		if HasFileSystem() and isfolder(self.Folder) then
			if delfolder then
